...
barplot(vetor_total_resultados, col = c(1,2,3,4,5)) #col é o vetor de cores, é só mudar os valores
...
# Barplot
ggplot(mtcars, aes(x = as.factor(cyl))) + #camada de dados
  geom_bar() #camada do gráfico
...
fatias <- c(4, 12, 14, 16, 8)
paises<- c("Brasil", "Estados Unidos", "Alemanha", "Reino Unido", "Espanha")
pie3D(fatias, labels = paises, explode = 0.05, main = "Leitura de Livros Por País")
...
# Plot 
ggplot(mpg, aes(x = reorder(class, hwy), y = hwy, fill = class)) + #camada 1 - dados
  geom_boxplot() + # camada 2 - gráfico
  xlab("class") + # camada 3 - label
  theme(legend.position = "none") #camada 4 - tema

##### Scatter Plot ##### Análise bivariada
...
ggplot(data, aes(x=my_x, y=my_y)) +
  geom_point(shape=1) +
  geom_smooth(method = lm, color="red", se=FALSE) #lm = linear model
